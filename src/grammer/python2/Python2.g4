/*
 * Parser grammar taken directly from official Python 2.7.13 grammar with only minor syntactical
 * changes (instances of [X] changed to (X)?, and semicolons added to ends of rules).:
 * https://docs.python.org/2/reference/grammar.html
 * 
 * Added lexer rules, and code to handle INDENT's, DEDENT's, line continuations, etc.
 * 
 ...
 * Grammar for Python
 * 
 * Note: Changing the grammar specified in this file will most likely require corresponding changes
 * in the parser module (../Modules/parsermodule.c). If you can't make the changes to that module
 * yourself, please co-ordinate the required changes with someone who can; ask around on python-dev
 * for help. Fred Drake <fdrake@acm.org> will probably be listening there.
 * 
 * NOTE WELL: You should also follow all the steps listed in PEP 306, "How to Change Python's
 * Grammar"
 * 
 * Start symbols for the grammar: single_input is a single interactive statement; file_input is a
 * module or sequence of commands read from an input file; eval_input is the input for the eval()
 * and input() functions. NB: compound_stmt in single_input is followed by extra NEWLINE!
 */
...
print_stmt:
	...
	// tt: this change allows print to be treated as a NAME while preserving the print statement
	// syntax.
	NAME (
		( test (',' test)* (',')?)?
		| '>>' test ( (',' test)+ (',')?)?
...
// Backward compatibility cruft to support: [ x for x in lambda: True, lambda: False if x() ] even
// while also allowing: lambda x: 5 if x else 2 (But not a mix of the two)
...
arglist: (argument ',')* (
	...
	// ambiguity. ast.c makes sure it's a NAME.;
